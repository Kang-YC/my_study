...
class IntegrationBase
{
  ...
  double ACC_W = 4.0e-5 ;//accelerometer bias random work noise standard deviation.  #0.02
  ...
  public:
    ...
    void repropagate(const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg)
    {
        ...
        for (int i = 0; i < static_cast<int>(dt_buf.size()); i++)
            propagate(dt_buf[i], acc_buf[i], gyr_buf[i]);//不同的偏置
    }
    ...
    {
        //ROS_INFO("midpoint integration"); 误差状态
        Vector3d un_acc_0 = delta_q * (_acc_0 - linearized_ba);
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);//四元数小角度近似
