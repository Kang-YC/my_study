...
#include <navigation_z.hpp>
#include <navigation_msg_z.hpp>
#include <bbox2d_z.hpp>
#include <bbox2i_z.hpp>
#include <bbox2f_z.hpp>
#include <bbox2f_z.hpp>
#include <point2d_z.hpp>
#include <point2i_z.hpp>
#include <pose_z.hpp>
//#include <pose.hpp>
...
zcm::ZCM zcm_sender{"udpm://239.255.76.67:7667?ttl=1"} ;
/**
...
void chatterCallback_lidar(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  navigation_msg_z lidar_pose;
 // pose msg_t;

  double roll, pitch, yaw, lidar_x, lidar_y;
  ...
  lidar_x = msg->pose.pose.position.z;
  lidar_y = -msg->pose.pose.position.x;
  ...
  //   latitude = 31.2304;
  //   longitude = 121.4337;
  //   latAngle.setByDegree(latitude);
  //   lonAngle.setByDegree(longitude);
  ...
  //   cout<<"utm: "<<UtmX  <<"   "<< UtmY <<endl;

    double latitude, longitude;
    double utmx =lidar_x + 350824;
    double utmy =lidar_y + 3456190;
    cout<<"utm: "<<utmx  <<"   "<< utmy <<endl;
    TiEV::WGS84Coor latLonPinvert;
    TiEV::UTMCoor coordinvert;
    coordinvert.x = utmx;
    coordinvert.y = utmy;
    latLonPinvert = TiEV::UTMXYToLatLon(coordinvert);
    latitude = latLonPinvert.lat.getDegree();
    longitude = latLonPinvert.lon.getDegree();

    lidar_pose.header.pose.heading = yaw;
    lidar_pose.data.latitude = latitude;
    lidar_pose.data.longitude = longitude;

    cout<<"latlon: "<<lidar_pose.data.latitude  <<"   "<< lidar_pose.data.longitude <<"  "<<lidar_pose.header.pose.heading<<endl;
    //msg_t.x = lidar_pose.data.latitude;
    zcm_sender.publish("HELLO_WORLD", &lidar_pose);

    //zcm_sender.publish("HELLO_WORLD", &msg_t);
